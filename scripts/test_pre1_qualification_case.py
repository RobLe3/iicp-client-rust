...
class DriverContractTests(unittest.TestCase):
    ...
    def test_referenced_test_files_exist(self) -> None:
        ...
        for command in commands:
            ...
            if "--test" in command:
                ...
                binary = command[command.index("--bin") + 1].replace("-", "_")
                source = ROOT / "src" / "bin" / f"{binary}.rs"
            else:
    ...
    def test_exact_discovery_refuses_zero_or_ambiguous_matches(self) -> None:
        ...
        self.assertFalse(module.exact_assertion_is_listed("0 tests, 0 benchmarks\n", assertion))
        self.assertFalse(
    ...
    def test_semantic_context_negative_controls_change_the_binding(self) -> None:
        base = (
            "client-rust|msrv-1.86|macos-arm64|rust|restricted",
            "rate-limit",
            "sha256:" + "a" * 64,
            "sha256:" + "b" * 64,
            "sha256:" + "c" * 64,
